...
推送 `main` 后，Actions 按顺序执行测试、类型检查、Lint、数据库迁移与版本校验、Next 构建、网关与 Worker 发布、健康检查、桌面/移动公开路由回归、真实账号会员壳层验收和真实发帖生命周期。会员验收覆盖个人主页 Hero、身份主题、好友/聊天窗口拖拽终止、窗口失焦与六档响应式。配置 `SUPABASE_DB_URL` 时 Actions 自动执行迁移；未配置时也必须先通过公开的精确 schema 版本校验，否则禁止发布。任一步失败都会恢复 Next 与网关的上一个软链接版本。生产数据库、用户上传文件和服务器密钥不进入发布包。
