...
test("public posts remain readable before the new profile RPC migration is applied", async () => {
  const rpcNames = [];
  const client = {
    from() {
      return {
        ...
        async range() {
          return {
            data: [{ id: "post-2", author_id: "user-2", post_images: [] }],
            error: null,
          };
        },
      };
    ...
    async rpc(name) {
      rpcNames.push(name);
      if (name === "get_public_post_profiles") {
        return { data: null, error: { message: "function not found" } };
      }
      return {
        data: [{ id: "user-2", public_uid: 583105, display_name: "Fallback" }],
        error: null,
      };
    },
  };

  const posts = await createCommunityRepository(client).listPosts("idea_sharing");

  assert.deepEqual(rpcNames, ["get_public_post_profiles", "get_public_profiles"]);
  assert.equal(posts[0].profiles.public_uid, 583105);
});
