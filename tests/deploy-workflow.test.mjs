...
test("backend deployment migrates only the exact predecessor schema before upload and rolls gateway code back on activation failure", () => {
  ...
  assert.match(backendSteps[schemaGate].run, /202609090002\)[\s\S]*202609100001_reward_lottery_manual_fulfillment\.sql/);
  assert.match(backendSteps[schemaGate].run, /202609100001\)[\s\S]*already applied/);
  assert.match(backendSteps[schemaGate].run, /Unexpected production schema marker; refusing migration/);
  assert.match(backendSteps[schemaGate].run, /test "\$schema_after" = 202609100001/);
  ...
  assert.match(backendSteps[publicSchemaCheck].run, /test "\$schema" = 202609100001/);
