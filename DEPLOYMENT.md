...
如需让 Actions 完成真实发帖全生命周期验收，在 GitHub `production` 环境增加专用测试账号 Secrets：`E2E_POSTING_IDENTIFIER` 和 `E2E_POSTING_PASSWORD`。测试通过 SSH 隧道访问回环预览服务，依次验证登录、发帖、图片上传、外链、详情刷新、作者主页、编辑、删图、删除帖子和退出；缺少凭据时只跳过这一步，不会使用普通用户账号。
