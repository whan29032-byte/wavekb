...
export function AdminRewards({ actorId, initialStore }: { actorId: string; initialStore: AdminRewardStore }) {
  ...
  const nameplateProducts = useMemo(() => store.products.filter((item) => item.product_type === "nameplate"), [store.products]);
  const wallets = useMemo(() => { const query = walletQuery.trim().toLowerCase(); return store.wallets.filter((item) => !query || [item.display_name, item.public_uid, item.display_title].some((value) => String(value || "").toLowerCase().includes(query))).slice(0, 100); }, [store.wallets, walletQuery]);
  ...
  return (
    <div className="grid gap-8">
      ...
      <section className="grid gap-4" aria-labelledby="reward-wallet-title"><header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><span className="flex items-center gap-2 text-xs font-semibold text-primary"><UserCircle aria-hidden size={17} />用户钱包</span><h2 id="reward-wallet-title" className="mt-1 text-xl font-semibold">调整积分并发放铭牌</h2><p className="mt-1 text-sm text-muted-foreground">积分原因进入用户账本；铭牌授权有明确有效期。</p></div><Input type="search" value={walletQuery} onChange={(event) => setWalletQuery(event.target.value)} placeholder="搜索昵称、UID 或称号" aria-label="搜索积分钱包" className="sm:max-w-sm" /></header><div className="grid gap-3">{wallets.length ? wallets.map((wallet) => <WalletRow key={`${wallet.user_id}:${wallet.updated_at}`} wallet={wallet} products={nameplateProducts} pending={pending.startsWith(`wallet:${wallet.user_id}`)} onAdjust={(delta, note) => safeRun(`wallet:${wallet.user_id}:points`, (actions) => actions.adjustPoints(wallet.user_id, delta, note), `已调整 ${wallet.display_name || "用户"} 的积分。`)} onGrant={(productId, days, equip) => safeRun(`wallet:${wallet.user_id}:plate`, (actions) => actions.grantNameplate(wallet.user_id, productId, days, equip), `已向 ${wallet.display_name || "用户"} 发放铭牌。`)} />) : <p className="rounded-xl border border-dashed bg-surface p-6 text-sm text-muted-foreground">没有匹配的积分账户。</p>}</div>{store.wallets.length > 100 && !walletQuery ? <p className="text-xs text-muted-foreground">默认显示最近更新的 100 个钱包，可搜索其余用户。</p> : null}</section>
