...
export function MobileNavigation() {
  ...
  return <div ref={root} className="md:hidden">
    ...
    {open ? <nav id={id} aria-label="移动主导航" className="absolute inset-x-4 top-full mt-2 grid gap-1 rounded-xl border bg-surface p-2 shadow-xl">
      {[["知识库", "/knowledge"], ["社区", "/community/idea_sharing"], ["导师", "/mentors"], ["积分商城", "/rewards"]].map(([label, href]) => <Link key={href} href={href} aria-current={pathname?.startsWith(href) ? "page" : undefined} onClick={() => setOpen(false)} className="flex min-h-11 items-center rounded-lg px-3 text-sm font-semibold hover:bg-muted aria-[current=page]:bg-muted aria-[current=page]:text-primary">{label}</Link>)}
    </nav> : null}
  </div>;
}
