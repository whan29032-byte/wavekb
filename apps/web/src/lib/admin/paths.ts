const readPaths = new Set(["users/summary", "users", "moderation-audit", "directory", "dashboard", "providers"]);
...
export function isAllowedAdminPath(path: string, method: "GET" | "POST") {
  return method === "GET" ? readPaths.has(path) : userMutationPath.test(path) || directoryMutationPath.test(path) || path === "providers";
}
